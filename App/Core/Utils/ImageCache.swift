    ...
    private func cleanExpiredCache() async {
        ...
        if totalSize > maxDiskCacheSize {
            let sortedFiles = files.sorted { file1, file2 in
                let dateKey: Set<URLResourceKey> = [.contentModificationDateKey]
                guard let date1 = try? file1.resourceValues(forKeys: dateKey).contentModificationDate,
                      let date2 = try? file2.resourceValues(forKeys: dateKey).contentModificationDate else {
                    return false
                }
                return date1 < date2
            }
